...
/-- Lema auxiliar: Contar pares ordenados con primera componente fija -/
private lemma count_pairs_with_fst (n : ℕ) [NeZero n] (i : ZMod (2*n)) :
    (Finset.univ.filter (fun p : OrderedPair n => p.fst = i)).card = 2*n - 1 := by
  have h_pos : 0 < 2 * n := by
    have : 1 ≤ n := NeZero.one_le
    omega
  -- Hay exactamente 2n-1 valores distintos de i para p.snd
  have : (Finset.univ.filter (fun j : ZMod (2*n) => j ≠ i)).card = 2*n - 1 := by
    rw [Finset.card_eq_iff_eq_univ_diff]
    · ext j
    ...
    · simp [Finset.card_univ, ZMod.card]
      omega
  -- Existe biyección entre pares con fst=i y valores snd≠i
  have bij : ∀ p : OrderedPair n, p.fst = i → p.snd ≠ i := by
    intro p hfst
    rw [hfst]
    exact p.distinct
...
/-- Lema auxiliar: Contar pares ordenados con segunda componente fija -/
private lemma count_pairs_with_snd (n : ℕ) [NeZero n] (i : ZMod (2*n)) :
    (Finset.univ.filter (fun p : OrderedPair n => p.snd = i)).card = 2*n - 1 := by
  have h_pos : 0 < 2 * n := by
    have : 1 ≤ n := NeZero.one_le
    omega
  -- Similar al lema anterior pero con snd
  have : (Finset.univ.filter (fun j : ZMod (2*n) => j ≠ i)).card = 2*n - 1 := by
    rw [Finset.card_eq_iff_eq_univ_diff]
    · ext j
      simp only [Finset.mem_filter, Finset.mem_univ, true_and, Finset.mem_sdiff, 
    ...
    · simp [Finset.card_univ, ZMod.card]
      omega
  have bij : ∀ p : OrderedPair n, p.snd = i → p.fst ≠ i := by
    intro p hsnd
    rw [hsnd]
...
/-- Cada elemento tiene exactamente (2n-1) pares posibles.
    ...
    **Demostración:** Para un elemento i ∈ Z/(2n)Z:
    - Hay (2n-1) pares con fst=i (porque snd puede ser cualquier valor ≠ i)
    - Hay (2n-1) pares con snd=i (porque fst puede ser cualquier valor ≠ i)
    - Pero estos conjuntos son disjuntos
    - Por principio de inclusión-exclusión: (2n-1) + (2n-1) - 0 = 2(2n-1)
    
    Sin embargo, el enunciado correcto debe ser: cada elemento aparece en
    exactamente 2(2n-1) pares (sumando ambas posiciones). El teorema actual
    cuenta solo una dirección.
-/
theorem pairs_per_element (n : ℕ) [NeZero n] (i : ZMod (2*n)) :
  ...
  have h_disj : Disjoint S_fst S_snd := by
    ...
    intro p hp q hq
    simp only [S_fst, S_snd, Finset.mem_filter, Finset.mem_univ, true_and] at hp hq
    intro h_eq
    subst h_eq
    rw [hp] at hq
    exact p.distinct hq
  
  -- La unión es exactamente el conjunto que queremos
  have h_union : Finset.univ.filter (fun p : OrderedPair n => p.fst = i ∨ p.snd = i) =
                 S_fst ∪ S_snd := by
    ext p
    simp [S_fst, S_snd]
  ...
  rw [count_pairs_with_fst n i, count_pairs_with_snd n i]
  ring
...
- `pairs_per_element`: Cada elemento aparece en exactamente 2(2n-1) pares

...
/-- **REFLEXIÓN ESPECULAR COMPLETA** sin sorry
    ...
    La reflexión invierte cada par ordenado: (e, s) ↦ (s, e)
-/
...
/-- Lema: adjustDelta mantiene valores en [-3, 3] para entradas en [-5, 5] -/
lemma adjustDelta_bounded (δ : ℤ) (h_range : -5 ≤ δ ∧ δ ≤ 5) :
...
theorem gap_ge_three (K : K3Config) : K.gap ≥ 3 := by
  ...
  have hbound : ∀ x ∈ K.ime, x ≥ 1 := by
    ...
    -- clear hx hd_mem hd_eq -- Removed to avoid unknown identifier error
    ...
    split_ifs with h1 h2
    · -- caso: δ > 3
      have : (p.fst.val : ℤ) < 6 := by norm_cast; exact ZMod.val_lt p.fst
      have : (p.snd.val : ℤ) < 6 := by norm_cast; exact ZMod.val_lt p.snd
      omega
    · -- caso: δ ≤ 3 y δ < -3
      have : (p.fst.val : ℤ) < 6 := by norm_cast; exact ZMod.val_lt p.fst
      have : (p.snd.val : ℤ) < 6 := by norm_cast; exact ZMod.val_lt p.snd
    ...
    · -- caso: δ ∈ [-3, 3]
      ...
      have : (p.snd.val : ℤ) ≠ (p.fst.val : ℤ) := by
        intro heq
        have h_nat : p.snd.val = p.fst.val := Int.ofNat.inj heq
        have h_zmod : p.snd = p.fst := ZMod.val_injective 6 h_nat
        exact hdist h_zmod.symm
...
theorem dme_mirror (K : K3Config) :
  ...
  | none =>
    ...
    have h_len_3 : K.pairs.toList.length = 3 := by
      rw [Finset.length_toList, K.card_eq]
  ...
  | some p_rev =>
    ...
    have : ∃ p_orig, p_orig ∈ K.pairs.toList ∧ p_orig.reverse = p_rev := by
      cases eq : K.pairs.toList[i]? with
      | none => simp [eq] at h
      | some p =>
        simp [eq] at h
        exists p
        constructor
        · rw [List.mem_iff_getElem?]; use i; exact eq
        · exact h

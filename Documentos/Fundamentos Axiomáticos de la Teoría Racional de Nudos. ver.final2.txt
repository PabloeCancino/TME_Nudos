...
### Una formalización algebraica basada en pares ordenados, estructuras modulares y operaciones internas
...
Este documento establece los **fundamentos formales**, **el núcleo axiomático**, **definiciones primitivas** y **operaciones básicas** que dan sustento a una teoría algebraico–topológica de nudos basada en:

1. pares ordenados racionales $\frac{o_i}{u_i}$;
2. la estructura modular sobre el anillo $\mathbb{Z}/_{2n}\mathbb{Z}$;
3. las operaciones internas  
   - **Progresión** (desplazamiento natural del recorrido),  
   - **Inversión** (cambio del nivel over/under);
4. y equivalencias inducidas por las movidas racionales de Reidemeister.
...
- $\mathbb{Z}/_{2n}\mathbb{Z}$: anillo modular de enteros módulo $2n$.
- Símbolos de relación:
  - $ = $: igualdad.
  - $ < $: orden sobre los índices del recorrido.
- Símbolos funcionales:
  - $\mathrm{o}(i)$: aparición "sobre" (_over_) del cruce $i$.
  - $\mathrm{u}(i)$: aparición "bajo" (_under_) del cruce $i$.
- Símbolos de operación:
  - $\mathcal{P}$: operación **Progresión**.
  - $\mathcal{I}$: operación **Inversión**.
- Símbolos estructurales:
  - $K$: una configuración racional.
  - $\frac{o_i}{u_i}$: el racional asociado al cruce $i$.
...
En este documento, usamos el término **"configuración racional"** o **"nudo racional"** en un sentido más amplio, refiriéndonos a cualquier nudo codificable mediante el sistema de pares ordenados $\frac{o_i}{u_i}$ y estructura modular $\mathbb{Z}_{2n}$.

> **ADVERTENCIA:** Nuestro uso de "racional" **no se limita** a los rational knots clásicos, aunque los contiene como caso particular. La terminología refleja la naturaleza aritmético-modular de la representación, no una restricción topológica a priori.


### **1.3.2. Relación con Nudos 2-Puente Clásicos**

El marco modular racional **incluye** todos los nudos 2-puente clásicos (rational knots de Conway), pero potencialmente abarca una clase más amplia.

**Proposición 1.3.1 (Inclusión de 2-bridge knots).**  
Todo nudo 2-puente clásico admite una representación como configuración racional en el sentido de este documento.

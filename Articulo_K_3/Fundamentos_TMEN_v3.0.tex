...
La representación modular de nudos descansa sobre tres principios
...
Teoría Modular Estructural de Nudos (TMEN).
...
El orden en el par ordenado no es algebraicamente intercambiable:\\
...
Una \textbf{movida R3 modular} consiste en reemplazar, dentro de una
configuración modular \(K\), el triple
...
la Teoría Modular Estructural de Nudos (TMEN).
...
(Reidemeister
Modular)}}{AXIOMA A4 --- Equivalencia Isotópica (Reidemeister Modular)}}\label{axioma-a4-equivalencia-isotuxf3pica-reidemeister-modular}

Existe una relación de equivalencia \(\sim\) sobre el conjunto de
configuraciones modulares, generada por: - Las movidas modulares
...
marco modular estructural (TMEN). No pretende \textbf{demostrar} dicho teorema,
...
modulares de R1, R2, R3 (definidas discretamente en Sección 3) capturan
...
La correspondencia entre equivalencia modular (\(\sim\)) y equivalencia
...
Equivalencia por movidas modulares de Reidemeister en configuraciones
...
modulares R1, R2, R3 corresponden localmente a movidas topológicas 3.
...
fundamentos teóricos del marco modular estructural (TMEN).
...
irreducible} de la Teoría Modular Estructural de Nudos (TMEN). Sin embargo, es importante
notar que:
...
modular}}{D1 --- Cruce modular}}\label{d1-cruce-modular}
...
modular}}{D2 --- Configuración modular}}\label{d2-configuracion-modular}

Una configuración modular es el conjunto\\
\[K = \left\{(o_1, u_1), \dots, (o_n, u_n)\right\},\]\\
...
mayoría de configuraciones modulares), los signos alternan sistemáticamente
...
El \textbf{grado} de una configuración modular \(K\) es el número de
...
modular \(K\) desplazando todas las posiciones en \(k\) unidades módulo
\(2n\):
...
\(\mathcal{P}(K) \sim K\) para toda configuración modular \(K\).
...
\textbf{Enunciado.}\\
Para cualquier configuración modular \(K\) con \(n\) cruces, las
...
Sea \(K\) una configuración modular con \(n\) cruces, y sea
...
\item
  ...
  configuración modular, cada par ordenado de cruce \((o_i, u_i)\)
...
Para toda configuración modular \(K\) se cumple:
\[ S(K)^\mathsf{T} = -S(K). \]

\subsubsection{\texorpdfstring{\textbf{Demostración}}{Demostración}}\label{demostraciuxf3n-2}

Sea \(K\) una configuración modular con \(n\) cruces.\\
...
Sea \(K\) una configuración modular con \(n\) cruces, escrita como \[
K = \left\{\, (o_1, u_1), (o_2, u_2), \dots, (o_n, u_n) \,\right\}
\] donde, por definición de configuración modular, se cumple: \[
...
La operación ``espejo modular'' definida cruce a cruce es una
...
modular}}{Teorema T4 --- Invarianza de I(K) y F(K) en la forma normal modular}}\label{teorema-t4-invarianza-de-ik-y-fk-en-la-forma-normal-modular}

Sea \(K\) una configuración modular y sea \(\mathrm{FN}(K)\) su forma normal
modular irreductible. Entonces:
...
Además, por el Teorema T5 (Reductibilidad hacia forma normal),
toda configuración modular \(K\) admite una forma normal
...
Sea \(K\) una configuración modular cualquiera. Consideremos un proceso
de reducción:
...
isotopía), definidos a partir de la forma normal modular.
\end{quote}

...
class ConfiguracionRacional:
    ...
    def __init__(self, pares_ordenados: List[List[int]], normalizar: bool = True):
        ...
        Args:
            ...
            normalizar: Si True, normaliza la configuración ordenando por min elemento
    ...
    def normalizar_configuracion(pares_ordenados: List[List[int]]) -> List[List[int]]:
        """
        Normaliza la configuración ordenando los pares por su elemento mínimo.
        
        Esto crea una representación canónica de la configuración:
        - Primer par: el que contiene el elemento más pequeño (usualmente el 0 o el 1)
        - Segundo par: el que contiene el siguiente elemento más pequeño no usado
        - Y así sucesivamente
        ...
        Ejemplo:
            ...
            - [5,2] → min = 2
            - [3,0] → min = 0
            Salida: [[3,0], [1,4], [5,2]]  (ordenado por min: 0, 1, 2)
        
        Args:
            pares_ordenados: Lista de pares [[o1,u1], [o2,u2], ...]
            
        Returns:
            Lista de pares ordenada por elemento mínimo
        """
        ...
        # Retornar solo los pares ordenados (sin el min)
    ...
    def es_reducible(self) -> bool:
        ...
        - Tiene pares R2: cruces consecutivos que se cancelan (bigon)
        
        Returns:
            True si es reducible, False si es irreducible
        """
        # Detectar cruces R1 (bucles)
        # En espacio cíclico ℤ/(2n)ℤ, debemos usar distancia modular
        # Ejemplo: en mod 6, la distancia entre 6 y 1 es min(|6-1|, 6-|6-1|) = min(5,1) = 1
        modulo = 2 * self.n
        
        for o, u in self.pares:
            distancia_lineal = abs(o - u)
            # Distancia cíclica: la menor entre ir directo o dar la vuelta
            distancia_ciclica = min(distancia_lineal, modulo - distancia_lineal)
            
            if distancia_ciclica == 1:
                return True
        
        # Detectar pares R2 (bigones)
        # Un par (ca, cb) es R2 si:
        # - Ady(oa, ob) y Ady(ua, ub) [adyacencia en ambas hebras]
        # - Un intervalo contiene al otro (forma un bigon)
        for i in range(len(self.pares)):
            for j in range(i + 1, len(self.pares)):
                ...
                if ady_over and ady_under:
                    # Para R2, verificar si un intervalo contiene al otro
                    # (NO solo interlazado estricto)
                    ai, bi = min(oi, ui), max(oi, ui)
                    aj, bj = min(oj, uj), max(oj, uj)
                    
                    # Bigon: un intervalo contiene completamente al otro
                    # Patrón: ai < aj < bj < bi  OR  aj < ai < bi < bj

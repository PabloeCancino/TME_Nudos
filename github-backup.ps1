...
try {
    ...
    if (-not (Test-Path $GitIgnorePath)) {
        ...
        # Mostrar cuántas reglas tiene el .gitignore
        $gitignoreLines = (Get-Content $GitIgnorePath | Where-Object { $_ -notmatch '^\s*#' -and $_ -notmatch '^\s*$' }).Count
        Write-Info "   Reglas de exclusión: $gitignoreLines"
    }
    ...
    # Git add respeta automáticamente el .gitignore
    git add -A
    Write-Log "Ejecutado: git add -A (respetando .gitignore)" "INFO"
